...
function Link({ destination, text, includeUnderline, animateUnderline }: LinkComponentProps) {
  ...
  return (
    <div className="flex flex-col justify-center gap-2">     
      <a className="flex gap-2 items-center font-(family-name:--THICCCBOI-Bold) text-(--neutral-800) text-[18px] leading-[20px] xs:text-[22px] xs:leading-[24px] lightenText" href={destination} target="_blank" rel="noopener noreferrer" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() =>setIsHovered(false)}>
        {text}
